...
def create_pyvis_network(G, node_filter=None, max_nodes=100):
    ...
    for node in G_filtered.nodes():
        ...
        if depth == 0:
            ...
            color = '#95e1d3'  # Light green for deeper levels
...
def show_network_graph(G):
    ...
    **How to use:**
    ...
    - 🎨 Colors indicate depth: Red (seed) → Teal (level 1) → Green (level 2+)
    """)
    ...
    with col2:
        max_nodes = st.slider("Max nodes to display", 50, 200, 100, 10)
